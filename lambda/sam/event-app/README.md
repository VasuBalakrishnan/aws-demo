...
```bash
event-app$ sam build --use-container
```
...
```bash
event-app$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
event-app$ sam logs -n HelloWorldFunction --stack-name event-app --tail
```
...
```bash
event-app$ pip install -r tests/requirements.txt --user
# unit test
event-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
event-app$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name event-app
```
