...
```bash
trader$ sam build --use-container
```
...
```bash
trader$ sam logs -n StockCheckerFunction --stack-name trader --tail
```
...
```bash
trader$ pip install -r tests/requirements.txt --user
# unit test
trader$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
trader$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name trader
```
